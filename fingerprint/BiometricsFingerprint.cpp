...
#define LOG_VERBOSE "vendor.samsung.hardware.biometrics.fingerprint@2.1-service.a70q"
...
SecBiometricsFingerprint::SecBiometricsFingerprint() : mClientCallback(nullptr) {
...
SecBiometricsFingerprint::~SecBiometricsFingerprint() {
    if (ss_fingerprint_close() != 0) {
        LOG(ERROR) << "Can't close HAL module";
    }
}

Return<RequestStatus> SecBiometricsFingerprint::ErrorFilter(int32_t error) {
    switch (error) {
        case 0:
...
FingerprintError SecBiometricsFingerprint::VendorErrorFilter(int32_t error, int32_t* vendorCode) {
    *vendorCode = 0;
    switch (error) {
...
FingerprintAcquiredInfo SecBiometricsFingerprint::VendorAcquiredFilter(int32_t info,
                                                                    int32_t* vendorCode) {
    *vendorCode = 0;
    switch (info) {
...
Return<uint64_t> SecBiometricsFingerprint::setNotify(
...
Return<uint64_t> SecBiometricsFingerprint::preEnroll() {
    return ss_fingerprint_pre_enroll();
}

Return<RequestStatus> SecBiometricsFingerprint::enroll(const hidl_array<uint8_t, 69>& hat,
...
Return<RequestStatus> SecBiometricsFingerprint::postEnroll() {
    return ErrorFilter(ss_fingerprint_post_enroll());
}

Return<uint64_t> SecBiometricsFingerprint::getAuthenticatorId() {
    return ss_fingerprint_get_auth_id();
}

Return<RequestStatus> SecBiometricsFingerprint::cancel() {
    return ErrorFilter(ss_fingerprint_cancel());
}

Return<RequestStatus> SecBiometricsFingerprint::enumerate() {
...
Return<RequestStatus> SecBiometricsFingerprint::remove(uint32_t gid, uint32_t fid) {
...
Return<RequestStatus> SecBiometricsFingerprint::authenticate(uint64_t operationId, uint32_t gid) {
    return ErrorFilter(ss_fingerprint_authenticate(operationId, gid));
}

Return<void> SecBiometricsFingerprint::request(int32_t cmd_id, int32_t len,
...
ISecBiometricsFingerprint* SecBiometricsFingerprint::getInstance() {
    if (!sInstance) {
        sInstance = new SecBiometricsFingerprint();
    }
    return sInstance;
}

bool SecBiometricsFingerprint::openHal() {
    ...
    if (handle) {
        ...
        if ((err = ss_set_notify_callback(SecBiometricsFingerprint::notify)) != 0) {
...
void SecBiometricsFingerprint::notify(const fingerprint_msg_t* msg) {
    SecBiometricsFingerprint* thisPtr =
        static_cast<SecBiometricsFingerprint*>(SecBiometricsFingerprint::getInstance());
